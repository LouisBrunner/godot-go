...
func ClassDBAddProperty(
	...
	var snSetterGDName StringName
	if setter != nil {
		snSetterGDName = NewStringNameWithLatin1Chars(setter.MethodName)
	} else {
		snSetterGDName = NewStringNameWithLatin1Chars("")
	}
...
func classDBBindMethod(
	...
	if methodMetadata.IsVirtual {
		if !strings.HasPrefix(goMethodName, "X_") {
			log.Panic(`virtual method name must have a prefix of "X_".`)
		}
	} else {
		if strings.HasPrefix(goMethodName, "X_") {
			log.Panic(`method name cannot have a prefix of "X_".`)
		}
	}
	mb := NewMethodBind(
		className,
